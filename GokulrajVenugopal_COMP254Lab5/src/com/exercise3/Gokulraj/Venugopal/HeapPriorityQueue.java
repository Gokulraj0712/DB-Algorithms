...
*/
   ...
   * @param integer n
   */
  protected void upheap(int n) // upHead Method
  {
      if (n <= 1) //instead of while loop, this will handle the while loop
      {
          return;
      }
      int a = parent(n);
      if (compare(heap.get(n), heap.get(a)) >= 1) 
      {
          return; // this is for break
      }
      swap(n, a);
      upheap(a); // This is the recursive method call
  }
