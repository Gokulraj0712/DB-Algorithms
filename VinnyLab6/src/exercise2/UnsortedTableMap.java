...
public class UnsortedTableMap<K,V> extends AbstractMap<K,V> {
  ...
  public static void main(String[] args) {
	  ...
	  System.out.println(map.containsKey("elderberry")); // false
}
  
}

/***
 * To implement the containsKey(k) method for the UnsortedTableMap class, we can simply call the findIndex(k) method and check if it returns a valid index. If the index is valid, then the key exists in the map and we can return true. Otherwise, the key does not exist in the map and we can return false.

*/

/***
To test the containsKey(k) method, we can create a UnsortedTableMap object and add some key-value pairs to it. Then, we can call the containsKey(k) method with some keys that exist in the map and some keys that do not exist in the map, and check if the method returns the correct result.
*/