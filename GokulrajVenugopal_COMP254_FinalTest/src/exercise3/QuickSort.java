...
class QuickSort {
  ...
  public static <K> void quickSort(Queue<Item> S, Comparator<Item> comp) {
    int n = S.size();
    if (n < 2) return;                       // queue is trivially sorted
    // divide
    Item pivot = S.first();                     // using first as arbitrary pivot
    Queue<Item> L = new LinkedQueue<>();
    Queue<Item> E = new LinkedQueue<>();
    Queue<Item> G = new LinkedQueue<>();
    while (!S.isEmpty()) {                   // divide original into L, E, and G
      Item element = S.dequeue();
  ...
  {
	  Queue<Item> S = new LinkedQueue<>();
	  S.enqueue(new Item(11, "Pen", 5.0));
	  S.enqueue(new Item(21, "Note-Book", 7.0));
	  S.enqueue(new Item(11, "FootBall", 250.0));
	  S.enqueue(new Item(31, "Dark Chocolate", 10.0));
	  S.enqueue(new Item(41, "Chips", 2.0));
	  S.enqueue(new Item(40, "Cricket Ball", 12.0));
	  S.enqueue(new Item(1, "Cricket Bat", 300.0));
	  S.enqueue(new Item(10, "Candy", 1.0));
