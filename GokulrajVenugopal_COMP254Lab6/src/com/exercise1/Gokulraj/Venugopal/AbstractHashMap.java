...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  private double maxLoadFactor;        // maximum allowed load factor
  ...
  { this(17,109345121,loadfactor); }                     // default capacity
  ...
  public V put(K key, V value) {
    ...
    if ((double) n / capacity > maxLoadFactor)              // keep load factor <= 0.5
      {
    	int newCap = 2 * capacity - 1;
    	if ((newCap & 1) == 0) newCap++; // ensure new capacity is odd
        resize(newCap);
      }
    return answer;
  }
